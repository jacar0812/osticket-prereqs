...
After applying the changes, we'll need to download a few things so OSticket can run on the VM. Click this link to download the necessary files: https://drive.google.com/drive/u/0/folders/1APMfNyfNzcxZC6EzdaNfdZsUwxWYChf6 . First, download and install PHP Manager for IIS and rewrite model. Make sure to download the files on Google Drive through your VM and not on your own computer. After downloading, create the directory C:\PHP.
<p>
...
Once this is done, go back to osTicket in the browser and begin setting it up. Create a username and password. Before filling out the next section, download HeidiSQL from the installation files. Once installed, click new and type in the password you created when you downloaded MySQL.
<p>
...
Lastly, we need to do some cleanup. We need to delete the setup folder in C:\inetpub\wwwroot\osTicket. We will also set permissions to "Read" only for the "ost-config.php" folder. After that, osTicket is ready to be used!
